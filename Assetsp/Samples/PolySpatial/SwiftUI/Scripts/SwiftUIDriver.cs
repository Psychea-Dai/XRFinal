    ...
    public class SwiftUIDriver : MonoBehaviour
    {
        ...
        static void CallbackFromNative(string command, int value)
        {
            ...
            try
            {
                ...
                var self = FindAnyObjectByType<SwiftUIDriver>();
#else
...
#if UNITY_6000_4_OR_NEWER
            SetLastObjectEntityId(EntityId.ToULong(thing.GetEntityId()));
#else
...
#if UNITY_6000_4_OR_NEWER
        [DllImport("__Internal")]
        static extern void SetLastObjectEntityId(ulong entityId);
#else
        [DllImport("__Internal")]
...
#if UNITY_6000_4_OR_NEWER
        static void SetLastObjectEntityId(ulong entityId) {}
#else
